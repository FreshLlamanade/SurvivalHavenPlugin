...
public class FixedLocationParticleGuide extends ParticleGuide {
    ...
    FixedLocationParticleGuide(SurvivalHavenPlugin plugin, Player player, Location target, Color color) {
    ...
    void show() {
        ...
        if (startLocation.distance(target) <= guides.length.get()) {
        ...
        for (int blocksAway = 1; blocksAway <= guides.length.get(); blocksAway++) {
